function[invP]=setupinvP2D(P,n,boxsize)
%n/boxsize needs to be a power of 2
invP(1:2*(n/boxsize)^2)=struct('A',[],'B',[],'C',[],'D',[],...
    'idxin',[],'idxbd',[]);
count=0;
L=log2(n/boxsize)+1;
idx=zeros([n+1,n+1]);
idx(1:n,1:n)=reshape(1:n^2,[n,n]);
idx(n+1,:)=idx(1,:);
idx(:,n+1)=idx(:,1);

%building correction matrix
%bdr1 or bdr2 will be taken care by others.
[bdr1,bdr2]=ndgrid([false(boxsize,1);true]);
idxbdr1=find(bdr1);
idxbdr2=find(bdr2);
...
for l=1
    ...
    [grid1,grid2]=ndgrid(buildgrid);
    curgrid=grid1|grid2;
    curin=false([cursize+1,cursize+1]);
    curin(2:cursize,2:cursize)=true;
    curbd=~curin;
    curgridin=curgrid&curin;
    curgridbd=curgrid&curbd;
    lclidx=zeros([cursize+1,cursize+1]);
    curmatsize=sum(curgrid(:));
    lclidx(curgrid)=1:curmatsize;
    lclidxin=lclidx(curgridin);
    lclidxbd=lclidx(curgridbd);
    boxidx=zeros([curnum,curnum]);
    for bigi2=1:curnum
        for bigi1=1:curnum
...
for l=2:L-1
    ...
    lclidxin=lclidx(curgridin);
    lclidxbd=lclidx(curgridbd);
    childrenidx=cell([2,2]);
    for c2=1:2
        for c1=1:2
            childgrid=false([cursize+1,cursize+1]);
            childgrid(1+(c1-1)*(cursize/2):1+c1*(cursize/2),1+(c2-1)*(cursize/2):1+c2*(cursize/2))=true;
            childrenidx{c1,c2}=lclidx(curgrid&childgrid);
        end
    end
    prevboxidx=boxidx;
    boxidx=zeros([curnum,curnum]);
    for bigi2=1:curnum
        for bigi1=1:curnum
            ...
            for c2=1:2
                for c1=1:2
                    curmat(childrenidx{c1,c2},childrenidx{c1,c2})=...
                        curmat(childrenidx{c1,c2},childrenidx{c1,c2})+...
                        invP(prevboxidx(2*bigi1+c1-2,2*bigi2+c2-2)).D;
                    invP(prevboxidx(2*bigi1+c1-2,2*bigi2+c2-2)).D=[];
                end
            end
            A=inv(curmat(lclidxin,lclidxin));
...
for l=L
    ...
    curgrid=grid1|grid2;
    curin=false([cursize+1,cursize+1]);
    curin(1:cursize,1:cursize)=true;
    curgridin=curgrid&curin;
    lclidx=zeros([cursize+1,cursize+1]);
