function varargout=jdqzFun(varargin)
%JDQZFUN computes a partial generalized Schur decomposition (or QZ
%  decomposition) of a pair of square matrices or operators.
%
%  LAMBDA=JDQZFUN(n,A,B) and JDQZFUN(n,A,B) return K eigenvalues of the matrix 
%  pair (A,B) of size n by n, where K=min(5,N) and N=size(A,1) if K has not
%  been specified.
%
%  [X,JORDAN]=JDQZFUN(n,A,B) returns the eigenvectors X and the Jordan
...
%
%  [X,JORDAN,HISTORY]=JDQZFUN(n,A,B) returns also the convergence history.
%
%  [X,JORDAN,Q,Z,S,T,HISTORY]=JDQZFUN(n,A,B)
...
%  JDQZFUN(n,A,B)
%  JDQZFUN(n,'Afun','Bfun')
%  The second input argument is either a square matrix (which can be full
...
%  columns of a given matrix, or a functional handle that gives the action
%  of the matrix A. In the second case, the M-file, say Afun.m, must return
%  the dimension n of the problem with n = Afun([],'dimension').
%  For example, JDQZFUN('fft',...) is much faster than JDQZFUN(F,...), where F is
...
%
%  [X,JORDAN,Q,Z,S,T,HISTORY] = JDQZFUN(n,A,B,K,SIGMA,OPTIONS)
%  [X,JORDAN,Q,Z,S,T,HISTORY] = JDQZFUN(n,'Afun','Bfun',K,SIGMA,OPTIONS)
%
%  where
%
...
%
%
%  JDQZFUN without input arguments returns the options and its defaults.
%

%   Gerard Sleijpen.
%   Copyright (c) 2002
%
%   Modified by Haizhao Yang for function handles
%   Copyright (c) 2016

global Qschur Zschur Sschur Tschur ...
    Operator_MVs Precond_Solves ...
    MinvZ QastMinvZ numIterGMRES numSolve
numIterGMRES = 0;
if nargin==0
...
if DISP & size(history,1)>0
    ...
    if mrs>0, rs=rs+0.1*eps*mrs;
        ...
        String=sprintf('JDQZFUN with jmin=%g, jmax=%g, residual tolerance %g.',...
            jmin,jmax,tol);
        title(String)
...
global Qschur Zschur Sschur Tschur numSolve Operator_MVs
...
global Qschur Zschur numIterGMRES
...
if ischar(Operator_A) | (isa(Operator_A,'function_handle') & ...
        isa(Operator_B,'function_handle'))
...
%
% This function is specialized for use in JDQZFUN.
...
Operator_A = varargin{2};
sizeA = varargin{1};

n=CheckMatrix(Operator_A,1);
if numel(n)>0
    if sizeA~=n
        error('input problem size is not equal to the matrix size');
    end
else
    n = sizeA;
end
...
if isempty(Operator_B)
    ...
    if isa(Operator_A,'function_handle') & ...
            isa(Operator_B,'function_handle')
        Operator_Form=10;
    end
end
...
if DISP
    ...
    if length(jj)>0 & (ischar(Operator_A) | ischar(Operator_B))
        ...
        fprintf(' The JDQZFUN input arguments, number%s, are\n',msgj)
