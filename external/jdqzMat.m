function varargout=jdqzMat(varargin)
%JDQZMAT computes a partial generalized Schur decomposition (or QZ
%  decomposition) of a pair of square matrices or operators.
%  
%  LAMBDA=JDQZMAT(A,B) and JDQZMAT(A,B) return K eigenvalues of the matrix pair
%  (A,B), where K=min(5,N) and N=size(A,1) if K has not been specified.
%  
%  [X,JORDAN]=JDQZMAT(A,B) returns the eigenvectors X and the Jordan
...
%  
%  [X,JORDAN,HISTORY]=JDQZMAT(A,B) returns also the convergence history.
%  
%  [X,JORDAN,Q,Z,S,T,HISTORY]=JDQZMAT(A,B) 
...
%  For example, JDQZMAT('fft',...) is much faster than JDQZMAT(F,...), where F is
...
%  
%  [X,JORDAN,Q,Z,S,T,HISTORY] = JDQZMAT(A,B,K,SIGMA,OPTIONS)
%  [X,JORDAN,Q,Z,S,T,HISTORY] = JDQZMAT('Afun','Bfun',K,SIGMA,OPTIONS)
%  
%  where
%  
...
%  
%
%  JDQZMAT without input arguments returns the options and its defaults.
%
...
if DISP & size(history,1)>0
  ...
  if mrs>0, rs=rs+0.1*eps*mrs;
    ...
    String=sprintf('JDQZMAT with jmin=%g, jmax=%g, residual tolerance %g.',...
            jmin,jmax,tol); 
    title(String) 
...
%
% This function is specialized for use in JDQZMAT.
...
if DISP 
  ...
  if length(jj)>0 & (ischar(Operator_A) | ischar(Operator_B))
    ...
    fprintf(' The JDQZMAT input arguments, number%s, are\n',msgj)
