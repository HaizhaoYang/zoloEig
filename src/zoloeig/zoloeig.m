function [Uout,eigvals] = zoloeig(A,varargin)
...
opt.cheborder = 50;
opt.ratio = 0.9;
slicing = MSSlicingCheb(A,a0,b0,a0,b0,a0,opt);
...
for cnt = 1 : nSlices
    ...
    if cnt == nSlices
        ...
        mlt = 1;
        while mlt < n
            evshift = eigs(A-shift*speye(n),4*mlt,'sm');
            if max(evshift)-min(evshift) > 1e-8
                break;
            end
            fprintf('Repeated %d eigenvalues = %e\n', mlt*4, shift);
            mlt = mlt*2;
        end
        evshift = sort([0;evshift]) + shift;
...
for cnt = 1:nSlices
    if nargout == 2
        [ Usub, evsub ] = zoloeigs( A, aint{cnt}, bint{cnt}, nc, ...
            r, reltol, itmax, verbose, itsolacc );
        evsub = diag(evsub);
        Uout(:,offset+(1:size(Usub,2))) = Usub;
    else
        evsub = zoloeigs( A, aint{cnt}, bint{cnt}, nc, ...
            r, reltol, itmax, verbose, itsolacc );
    end
