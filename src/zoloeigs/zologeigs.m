function [Uout,eigvals,relerrs] = zologeigs(A,B,a,b,varargin)
...
for it = 1:r(1)
    ...
    prefact{it} = Multifrontal((A-sigmai*B)/wi,SMF);
end
...
R = @(X)applyZoloMat(A,B,cc,prefact,X);
...
for it = 1:itmax
    ...
    Xi = X(:,idx);
    D = D(idx,idx);
    relerr = norm(A*Xi - B*Xi*D, 'fro')/norm(A*Xi, 'fro');
    relerrs(it) = relerr;
