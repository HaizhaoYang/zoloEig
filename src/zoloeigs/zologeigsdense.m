function [Uout,eigvals,relerrs] = zologeigsdense(Afun,GenAinvfun,Bfun,n,a,b,varargin)

% GenAinvfun(a) generates the inverse of (Afun-a*Bfun) in the form of a
% function handle
...
if length(a) == 2 && length(b) == 2
    ...
    [a1,a2,b1,b2] = eiggaps(Afun,Bfun,n,a,b);
end
...
R = @(X)applyZoloDenseMat(cc,Ainvzolo,X,Bfun);
...
for it = 1:itmax
    ...
    Xi = X(:,idx);
    D = D(idx,idx);
    relerr = norm(Afun(Xi) - Bfun(Xi)*D, 'fro')/norm(Afun(Xi), 'fro');
    relerrs(it) = relerr;
