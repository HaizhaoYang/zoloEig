...
hold all;
errorbar(siz,mean(timeig),mean(timeig)-min(timeig),max(timeig)-mean(timeig));
%errorbar(siz,mean(timfeast),mean(timfeast)-min(timfeast),max(timfeast)-mean(timfeast));
errorbar(siz,mean(timzolo),mean(timzolo)-min(timzolo),max(timzolo)-mean(timzolo));
plot(siz,min(min(timzolo))/min(siz)*siz,':');
plot(siz,min(min(timzolo))/min(siz)^2*siz.^2,':');
xlabel('Size');
ylabel('Time');
legend('Eig time','ZoloEig time','linear ref','quadratic ref');

figure(2)
set(gca,'XScale','log','YScale','log');
hold all;
%errorbar(siz,mean(errfeast),mean(errfeast)-min(errfeast),max(errfeast)-mean(errfeast));
errorbar(siz,mean(errzolo),mean(errzolo)-min(errzolo),max(errzolo)-mean(errzolo));
xlabel('Size');
